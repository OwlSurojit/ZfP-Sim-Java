    ...
    public void mouseClicked(MouseEvent me) {
        if(me.getButton() == 1){
            if (start == null){
                ...
                Point sp = Raytracer.getLineIntersection(new Ray(corner2, (new Line(start, center1).toVector().toNormal())), new Line(start, corner1)).point;
                corner2 = (new Ray(start, (new Line(sp, p)).toVector())).getPoint(1);
                Point corner3 = (new Ray(corner1, (new Line(sp, p)).toVector())).getPoint(1);
                Point[] points = {start, corner1, corner2, corner3};
                ArrayList<ShapeBase> sb = new ArrayList<ShapeBase>();
                sb.add(new Polygon(points));
