...
public class Raytracer {
	...
	public Point getIntersection(Object obj) {
		...
		if (factors[0] != null && factors[1] != null)
			S = this.ray.getPoint(Math.min(factors[0], factors[1]));
		else if (factors[0] != null)
			S = this.ray.getPoint(factors[0]);
		else if (factors[1] != null)
	...
	public static StructPointBool getLineIntersection(Ray ray, Line line) {
		Ray v = line.toRay();
		if (ray.r.equals(v.r))
			return new StructPointBool(null, false);
		else {
			...
			return new StructPointBool(v.getPoint(factor),
					0 <= factor && factor <= 1 || Tools.equal(0, factor) || Tools.equal(1, factor));
		}
	}
	...
	public static Double[] getCircleIntersection(Ray ray, Circle circle) {
		...
		double q = (Math.pow(ray.o.x, 2) + Math.pow(ray.o.y, 2) + Math.pow(circle.center.x, 2)
				+ Math.pow(circle.center.y, 2) - 2 * (ray.o.x * circle.center.x + ray.o.y * circle.center.y)
				- Math.pow(circle.radius, 2)) / (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2));
		if (Math.pow(p / 2, 2) - q <= 0) {
			return new Double[] { null, null };
		} else {
			Double factor1 = -p / 2 + Math.sqrt(Math.pow(p / 2, 2) - q);
			Double factor2 = -p / 2 - Math.sqrt(Math.pow(p / 2, 2) - q);
			if (factor1 < 0 || Tools.equal(factor1, 0))
				factor1 = null;
			if (factor2 < 0 || Tools.equal(factor2, 0))
				factor2 = null;
			return new Double[] { factor1, factor2 };
		}
	}
	...
	public static Double[] getCircleArcIntersection(Ray ray, CircleArc arc) {
		...
		if (factors[0] != null) {
			...
			Vector S_offset = new Vector(Math.cos(Math.toRadians(arc.offsetangle)),
					-Math.sin(Math.toRadians(arc.offsetangle)));
			if (S_offset.getDirAngle((new Line(arc.center, S)).toVector()) > arc.arcangle)
				factors[0] = null;
			//            // Winkel zwischen Start und SP
			//            //Problem: gilt nur, wenn Vector(1,0) auf offenen Teil zeigt
			//            intersecangle = (new Vector(1,0).getDirAngle((new Line(arc.center, S)).toVector())) - arc.offsetangle;
	...
	public static Double[] getOvalIntersection(Ray ray, Oval oval) {
		Double factor1 = ((-(oval.e
				* Math.sqrt(Math.pow(oval.e, 4) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
						- 2 * Math.pow(oval.e, 2)
								* (2 * Math.pow(ray.o.x, 2) * Math.pow(ray.r.y, 2)
										- 2 * ray.o.x
												* (2 * ray.o.y * ray.r.x + oval.p1.x * ray.r.y - oval.p1.y * ray.r.x
														+ oval.p2.x * ray.r.y - oval.p2.y * ray.r.x)
												* ray.r.y
										+ 2 * Math.pow(ray.o.y, 2) * Math.pow(ray.r.x, 2)
										+ 2 * ray.o.y
												* (oval.p1.x * ray.r.y - oval.p1.y * ray.r.x + oval.p2.x * ray.r.y
														- oval.p2.y * ray.r.x)
												* ray.r.x
										+ Math.pow(oval.p1.x, 2) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
										- 2 * oval.p1.x * (oval.p2.x * ray.r.x + oval.p2.y * ray.r.y) * ray.r.x
										+ Math.pow(oval.p1.y, 2) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
										- 2 * oval.p1.y * (oval.p2.x * ray.r.x + oval.p2.y * ray.r.y) * ray.r.y
										+ (Math.pow(oval.p2.x, 2) + Math.pow(oval.p2.y, 2))
												* (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2)))
						+ (4 * Math.pow(ray.o.x, 2) * Math.pow(ray.r.y, 2)
								- 4 * ray.o.x
										* (2 * ray.o.y * ray.r.x + oval.p1.x * ray.r.y - oval.p1.y * ray.r.x
												+ oval.p2.x * ray.r.y - oval.p2.y * ray.r.x)
										* ray.r.y
								+ 4 * Math.pow(ray.o.y, 2) * Math.pow(ray.r.x, 2)
								+ 4 * ray.o.y
										* (oval.p1.x * ray.r.y - oval.p1.y * ray.r.x + oval.p2.x * ray.r.y
												- oval.p2.y * ray.r.x)
										* ray.r.x
								+ Math.pow(oval.p1.x, 2) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
								- 2 * oval.p1.x
										* (oval.p2.x * (Math.pow(ray.r.x, 2) - Math.pow(ray.r.y, 2))
												+ 2 * oval.p2.y * ray.r.x * ray.r.y)
								+ Math.pow(oval.p1.y, 2) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
								- 2 * oval.p1.y
										* (2 * oval.p2.x * ray.r.x * ray.r.y
												- oval.p2.y * (Math.pow(ray.r.x, 2) - Math.pow(ray.r.y, 2)))
								+ (Math.pow(oval.p2.x, 2) + Math.pow(oval.p2.y, 2))
										* (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2)))
								* (Math.pow(oval.p1.x, 2) - 2 * oval.p1.x * oval.p2.x + Math.pow(oval.p1.y, 2)
										- 2 * oval.p1.y * oval.p2.y + Math.pow(oval.p2.x, 2) + Math.pow(oval.p2.y, 2)))
				+ Math.pow(oval.e, 2) * (2 * ray.o.x * ray.r.x + 2 * ray.o.y * ray.r.y - oval.p1.x * ray.r.x
						- oval.p1.y * ray.r.y - oval.p2.x * ray.r.x - oval.p2.y * ray.r.y)
				- (2 * ray.o.x * (oval.p1.x - oval.p2.x) + 2 * ray.o.y * (oval.p1.y - oval.p2.y)
						- Math.pow(oval.p1.x, 2) - Math.pow(oval.p1.y, 2) + Math.pow(oval.p2.x, 2)
						+ Math.pow(oval.p2.y, 2))
						* (oval.p1.x * ray.r.x + oval.p1.y * ray.r.y - oval.p2.x * ray.r.x - oval.p2.y * ray.r.y)))
				/ (2 * (Math.pow(oval.e, 2) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2)) - Math.pow(
						oval.p1.x * ray.r.x + oval.p1.y * ray.r.y - oval.p2.x * ray.r.x - oval.p2.y * ray.r.y, 2))));
		Double factor2 = ((oval.e
				* Math.sqrt(Math.pow(oval.e, 4) * (Math.pow(ray.r.x, 2) + Math.pow(ray.r.y, 2))
						- 2 * Math.pow(oval.e, 2)
								* (2 * Math.pow(ray.o.x, 2) * Math.pow(ray.r.y, 2)
										- 2 * ray.o.x
	...
	public StructRayObj getClosestReflection() {
		...
		for (Object obj : this.objects) {
			...
			if (S != null && !S.equals(this.ray.o)) {
				...
				if ((this.ray.r.x * direction.x >= 0 || Tools.equal(direction.x, this.ray.r.x))
						&& (this.ray.r.y * direction.y >= 0 || Tools.equal(direction.y, this.ray.r.y))
						&& Tools.equal(distance, mindistance)) {
					if (cornerObjs.isEmpty())
						cornerObjs.add(closestObj);
					cornerObjs.add(obj);
					hitCorner = true;
				} else if ((this.ray.r.x * direction.x >= 0 || Tools.equal(direction.x, this.ray.r.x))
						&& (this.ray.r.y * direction.y >= 0 || Tools.equal(direction.y, this.ray.r.y))
		...
		if (hitCorner)
			return new StructRayObj(getCornerReflection(cornerObjs, getIntersection(cornerObjs.get(0))),
					cornerObjs.get(0));
		else
	...
	public Ray getCornerReflection(ArrayList<Object> cornerObjs, Point S) {
		...
		for (Object obj : cornerObjs) {
			...
			if (Line.class.isInstance(obj)) {
				Line line = (Line) obj;
				if (S.equals(line.start))
					vectors.add(line.toVector());
				else if (S.equals(line.end))
				...
				CircleArc arc = (CircleArc) obj;
				Point c1 = new Point(Math.cos(Math.toRadians(arc.offsetangle)),
						-Math.sin(Math.toRadians(arc.offsetangle)));
				Point c2 = new Point(Math.cos(Math.toRadians(arc.offsetangle + arc.arcangle)),
						-Math.sin(Math.toRadians(arc.offsetangle + arc.arcangle)));
				if (S.equals(c1)) {
		...
		} else if (this.ray.r.getDirAngle(minVec) > 180 && this.ray.r.getDirAngle(maxVec) > 180
				|| this.ray.r.getDirAngle(minVec) < 180 && this.ray.r.getDirAngle(maxVec) < 180) {
	...
	public Ray getCircleReflection(Ray ray, Circle circle, Point S) {
		...
		if (rt.r.equals(this.ray.r))
			return this.ray; // Ray ist Tangente -> keine Ref
