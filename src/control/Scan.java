...
public class Scan {
	...
	public Double[][] MultiScan_A(int numray, double angle) {
		if (numray < 2 || Tools.equal(angle, 0))
			return scan_A();
	...
	public Double[][] processScan_A2(Double[][] scan, int numray) {
		...
		for (int i = 1; i < scan.length; i++) {
			if (scan[i][0] - scan[i - 1][0] < avgtimedif)
				lower++;
			else
				higher++;
		}
	...
	private Object closest() {
		...
		for (Object object : this.objects) {
			...
			if (Line.class.isInstance(object)) {
				...
				StructPointBool intersec = getLineIntersection(new Ray(this.sender.ray.o, line.toVector().toNormal()),
						line);
				if (intersec.bool) {
					if (intersec.point.equals(this.sender.ray.o))
						length = 0;
				...
				Oval[] ovals = new Oval[] { new Oval(oval1.p1, oval1.p2, oval1.e + 1),
						new Oval(oval1.p1, oval1.p2, oval1.e - 1) };
				java.awt.Shape[] ovals2d = new java.awt.Shape[2];
				for (int i = 0; i < 2; i++) {
					Oval oval = ovals[i];
					AffineTransform at = new AffineTransform();
					at.setToRotation(
							-Math.toRadians((new Vector(1, 0)).getDirAngle((new Line(oval.p1, oval.p2).toVector()))),
							(oval.p1.x + oval.p2.x) / 2, (oval.p1.y + oval.p2.y) / 2);
					Ellipse2D.Double oval2d = DrawPanel.getOval2D(oval);
					ovals2d[i] = at.createTransformedShape(oval2d);
				}
				if (ovals2d[0].contains(this.ray.o.x, this.ray.o.y)
						&& !ovals2d[1].contains(this.ray.o.x, this.ray.o.y)) {
					length = 0;
				}
			}
