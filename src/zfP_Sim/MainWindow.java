...
public class MainWindow extends BodyWindow {
	...
	private void initComponents() {
		...
		measuringRunButton = new javax.swing.JButton();
		...
		degreeLabel.setText("Eingangswinkel in Â°");
		...
		angleLabel.setText("Streuungswinkel in Â°");
		...
		simToolBar.add(jSeparator8);

		// Sample count input [Semi 20/21]
		sampleCountLabel.setText("Anzahl an Samples");
		simToolBar.add(sampleCountLabel);

		sampleCountField.setText("1024");
		sampleCountField.setMinimumSize(new java.awt.Dimension(48, 26));
		...
		exportDataButton.setMaximumSize(new java.awt.Dimension(120, 26));
		exportDataButton.setMinimumSize(new java.awt.Dimension(120, 26));
		exportDataButton.setPreferredSize(new java.awt.Dimension(120, 26));
		...
		sampleRateField.setMinimumSize(new java.awt.Dimension(48, 26));
		sampleRateField.setPreferredSize(new java.awt.Dimension(48, 26));
		simToolBar.add(sampleRateField);

		endXLabel.setText("Sender x Endwert");
		simToolBar.add(endXLabel);

		endXField.setText("30");
		endXField.setMinimumSize(new java.awt.Dimension(48, 26));
		endXField.setPreferredSize(new java.awt.Dimension(48, 26));
		simToolBar.add(endXField);

		measuringRunButton.setText("Messfahrt starten");
		measuringRunButton
				.setBorder(javax.swing.BorderFactory.createBevelBorder(javax.swing.border.BevelBorder.RAISED));
		measuringRunButton.setFocusable(false);
		measuringRunButton.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
		measuringRunButton.setMaximumSize(new java.awt.Dimension(120, 26));
		measuringRunButton.setMinimumSize(new java.awt.Dimension(120, 26));
		measuringRunButton.setPreferredSize(new java.awt.Dimension(120, 26));
		measuringRunButton.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
		measuringRunButton.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				measuringRunActionPerformed(evt);
			}
		});
		simToolBar.add(measuringRunButton);
		...
		simPanelLayout.setHorizontalGroup(simPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
				.addGap(0, 1770, Short.MAX_VALUE));
		simPanelLayout.setVerticalGroup(simPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
		...
		scanPanelLayout.setHorizontalGroup(scanPanelLayout
				.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 1776, Short.MAX_VALUE));
		scanPanelLayout.setVerticalGroup(scanPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
		...
		newPregenMenuItem.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_P,
		...
		closeMenuItem.setText("SchlieÃŸen");
		...
		exportPDFMenuItem.setAccelerator(
		...
		// Export Scan Data [Semi 20/21]
		/*
		 * exportScanMenu.setText("Messdaten Exportieren"); exportScanMenu.addActionListener(new
		 * java.awt.event.ActionListener() { public void actionPerformed(java.awt.event.ActionEvent evt) {
		 * exportScanActionPerformed(evt); } }); fileMenu.add(exportScanMenu); fileMenu.add(jSeparator41);
		 */
		...
		layout.setVerticalGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING).addComponent(
				jSplitPane1, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, 1019,
				Short.MAX_VALUE));
	...
	private void simStartButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_simStartButtonActionPerformed
		if (!body.outline.isEmpty()) {
			...
			Sender sender = new Sender(
					new Ray(new Point(Double.parseDouble(senderXField.getText()),
							Double.parseDouble(senderYField.getText())), new Vector(rayX, rayY)),
					Double.parseDouble(rangeField.getText()));
			Scan scan = new Scan(body, sender, Integer.parseInt(refField.getText()),
					Double.parseDouble(velocityField.getText()), 0);
			if (Integer.parseInt(numRayField.getText()) == 1) {
				...
				simPanel.simulate(scan.MultiReflections(Integer.parseInt(numRayField.getText()),
						Double.parseDouble(angleField.getText())), i - 1);
				// scanPanel.setScores(scan.MultiScan_A(Integer.parseInt(numRayField.getText()) ,
				// Double.parseDouble(angleField.getText())));
				// scanPanel.setScores(scan.processScan_A(scan.MultiScan_A(Integer.parseInt(numRayField.getText()) ,
				// Double.parseDouble(angleField.getText())), 0.5));
				scanPanel.setScores(scan.processScan_A3(scan.MultiScan_A(Integer.parseInt(numRayField.getText()),
						Double.parseDouble(angleField.getText()))));
			}
		}
	...
	/**
	 * Called on pressing the Export button, exports simulated data using the {@code exportScans} package.
	 * 
	 * @param evt The called event
	 * 
	 * @author Owl Surojit
	 * @since Semi 2020/21
	 */
	private void exportScanActionPerformed(java.awt.event.ActionEvent evt) {
		JFileChooser jfc = new JFileChooser("../Messdaten/Simuliert");
		FileNameExtensionFilter f = new FileNameExtensionFilter("Textdateien", "txt");
		jfc.setFileFilter(f);
		jfc.setSelectedFile(new File("export.txt"));
		if (jfc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			String filename = jfc.getSelectedFile().getPath();
	...
	private void measuringRunActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (jfc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			String filename = jfc.getSelectedFile().getPath();
			exportScans.MeasuringRun.mr2d(body, simPanel, scanPanel, (int) Double.parseDouble(senderXField.getText()),
					(int) Double.parseDouble(endXField.getText()), (int) Double.parseDouble(senderYField.getText()),
	...
	private javax.swing.JButton measuringRunButton;
	...
	private static zfP_Sim.ScanPanel scanPanel;
	...
	public void getSenderPositions() {
		if (!body.outline.isEmpty()) {
			...
			senderPositions = new double[][] { new double[] { 300, 30 } };
		setIndex(0);
	}
	...
	// Added for Semi 2020/21
	public static zfP_Sim.ScanPanel getScanPanel() {
		return scanPanel;
	}
